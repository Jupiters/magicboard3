...
class Util
{
	...
	// 시간관련 함수
	static function GetDatetime($timestamp=0) {
		$datetime='';
		if($timestamp==0) {
	...
	// 시간의 역저장
	const time_ReverseValue = 3000000000; // 30억
	static function GetReverseTime($timestamp) {
		return self::time_ReverseValue-intval($timestamp);
	}
	...
	//한글 지원되는 자동링크
	static function auto_link($contents) {
		// domain+port
		...
		// parameters
		$pattern .= "(\?[_0-9a-z#%&=\-\+]+)*/i";
		$replacement = "<a href=\"\\0\" onclick=\"window.open(this.href); return false;\" >\\0</a>";
		return preg_replace($pattern, $replacement, $contents, -1);
	}

	// 변수를 디버그 하기 좋게 출력해줌..
	static function Dbg($value) {
		ob_start();
		var_dump($value);
	...
	// attribute를 받을 때에 기본 값으로 설정해주기 위한 도구
	static function SetDefault(&$change_array, $key, $default_value) {
		if(!is_array($change_array)) Dialog::alert("\$change_array, 배열이 아닙니다.\n배열을 첫번째 인자로 넣어주세요.");
		if(!isset($change_array[$key])) $change_array[$key] = $default_value;
	}	
	
	// 객체검사
	public static function CheckObj($obj, $name) {
		if(!(is_object($obj) && get_class($obj)==$name)) return false;
		return true;
	}
	
	// is msie
	public static function msie() {
		preg_match('/msie/', strtolower($_SERVER['HTTP_USER_AGENT']), $matches, PREG_OFFSET_CAPTURE, 3);
		if(sizeof($matches)!=0) return true;
		return false;
	}

	// is msie 5.5
	public static function msie5() {
		preg_match('/msie/', strtolower($_SERVER['HTTP_USER_AGENT']), $matches, PREG_OFFSET_CAPTURE, 3);
		if(sizeof($matches)!=0) {
			preg_match('/5.5/', strtolower($_SERVER['HTTP_USER_AGENT']), $matches, PREG_OFFSET_CAPTURE, 3);
			if(sizeof($matches)!=0) return true;
		}
		return false;
	}
	
	static function CheckMobile($iphone=true,$android=true,$opera=true,$blackberry=true,$palm=true,$windows=true,$mobileredirect=false,$desktopredirect=false){
