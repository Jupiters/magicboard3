...
/*
 * 수정
 * kevinpark1981@gmail.com 똥싼너구리(박경종)
 * 2012-03-07 - 매직보드에 맞도록 수정 / 추후 완전 모듈화 시키는것도 고려해보자
 * 				지금은 경로 및 옵셙설정등 문제점이 많다.
 */
...
if ( isset($_GET['zsfimg']) ) {
	...
	$patterns[0] = '/[0-9abcdef]{6}/i';
	$replacements[0] = '';
	if ( isset($zsfCfg['border-color']) && preg_replace($patterns, $replacements, $zsfCfg['border-color'])=='' ) { $thisZsfCfg['border-color'] = $zsfCfg['border-color']; }
	...
	# kevinpark1981@gmail.com //-> 경로문제는 나중에 다시 재설계 해야함
	if ( defined('zsfSessId') && is_file( '../../../../data/zmLogConnect/'.zsfSessId.'.php' ) && !isset($_GET['re']) && !isset($_GET['cfg']) ) {
		include '../../../../data/zmLogConnect/'.zsfSessId.'.php';
	}
	else {
		# 경우에 따라 문제 만들기
...
/* 검증 함수 시작 */
{
	function zsfCheck ( $zsfCode, $zsfLog='', $zsfMethod='POST' ) {
		...
		function zsfLog ( $zsfLogFile,  $zsfQ, $zsfA, $zsfCode, $zsfMethod, $zsfLog, $zsfTerm ) {
			$zsfLogFile = zsfAr.'../../../../data/zmLog/'.$zsfLogFile.'.php';
			$zsfLogDataArr = array ();
			$i = 0;
		...
		# 세션파일 있을 경우 INCLUDE
		# kevinpark1981@gmail.com
		if ( defined( 'zsfSessId' ) && is_file( zsfAr.'../../../../data/zmLogConnect/'.zsfSessId.'.php' ) ) {
